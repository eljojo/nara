...
func TestDelayedMissingReporting(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		ln := NewLocalNara("observer-"+string(rune('a'+i)), "test-soul", "host", "user", "pass", 50, 1000)
...
func TestDelayedMissingReporting_NoRedundancy(t *testing.T) {
	...
	ln1 := NewLocalNara("observer-fast", "test-soul", "host", "user", "pass", 50, 1000)
	...
	ln2 := NewLocalNara("observer-slow", "test-soul", "host", "user", "pass", 50, 1000)
