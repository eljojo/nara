...
- **plaza ([MQTT](/docs/spec/plaza-mqtt/))**: a lightweight message queue where naras broadcast announcements. anyone listening hears everything.
- **mesh (HTTP over [WireGuard](/docs/spec/mesh-http/))**: peer-to-peer connections using [Headscale](https://headscale.net/). direct nara-to-nara paths for gossip and private sync.
...
every nara's identity flows from its hardware through a deterministic chain. see the **[Identity Specification](/docs/spec/identity/)** for the full derivation logic.
...
sequenceDiagram
    ...
    A->>ME: howdy! here's what you missed 📦
    B->>ME: howdy! here's what you missed 📦
    C->>ME: howdy! here's what you missed 📦
...
events are signed by their creator, timestamped, and immutable. the ledger doesn't store "current state", it stores "what happened". state is always derived. see the **[Events Specification](/docs/spec/events/)** for schema details and **[Service Types](/docs/spec/events/#service-types)** for the importance levels.
...
sequenceDiagram
    ...
    Note over PROJ: status: 🟢 online

    EVENTS->>PROJ: "nara b went offline"
    Note over PROJ: status: 🔴 offline<br/>downtime: 0s

    Note over EVENTS,PROJ: 3 minutes pass...

    EVENTS->>PROJ: "nara b came online"
    Note over PROJ: status: 🟢 online<br/>last downtime: 3m<br/>restarts: +1
```

events are **immutable facts**. the projection is **computed state** that updates as new events arrive. nothing is stored as "current status"—it's always derived from the event history. see **[Projections](/docs/spec/projections/)**.
...
### stash: storage without disks 🔐

stash is **distributed encrypted storage**. instead of saving to disk, a nara keeps its private state on trusted peers (confidants). think of it like leaving copies of your diary with friends. see **[Stash Specification](/docs/spec/stash/)**.
...
flowchart TB
    subgraph "storing"
        ...
        ME -->|"encrypt with<br/>my symmetric key"| ENCRYPTED["🔒 encrypted stash"]
...
flowchart TB
    subgraph "recovering (on boot)"
        ...
        RECOVER -->|"decrypt with<br/>my key"| RESTORED["🔓 my stuff restored"]
    end
...
### uptime: who's watching whom ⏱️

naras track each other's uptime through observations. no central authority decides who's online—everyone watches and forms consensus. see **[Observations](/docs/spec/observations/)** and **[Presence](/docs/spec/presence/)**.
...
sequenceDiagram
    ...
    A->>B: here's my vote ✍️
    B->>C: here's my vote ✍️
    C->>D: here's my vote ✍️
    D->>A: here's my vote ✍️

    Note over A,D: collect votes, compute consensus

    A->>A: trimmed mean: 46.5 uptime<br/>majority: 3 restarts
    A->>A: sign checkpoint ✍️
```

the process:
1. each nara shares their **observed values** for every peer (uptime, restarts, etc.)
2. votes are collected and **outliers are trimmed** (protection against bad actors) 🗳️
...
flowchart LR
    subgraph "checkpoint"
        ...
        SIG_A["signature from a ✍️"]
        SIG_B["signature from b ✍️"]
        SIG_C["signature from c ✍️"]
    end

    DATA --> ANCHOR["historical anchor 🔏<br/>(can't be disputed)"]
    SIG_A --> ANCHOR
    SIG_B --> ANCHOR
    SIG_C --> ANCHOR
...
a checkpoint signed by multiple naras is **hard to dispute**. it's not one nara's opinion—it's a collective agreement, cryptographically sealed. "how long has pixel been alive?" stops being gossip and becomes historical fact. see **[Checkpoints](/docs/spec/checkpoints/)**.

### zines: gossip in bundles 📰

naras compile "zines" with whatever updates they've seen and exchange them with each other. it's like passing around a hand-made newsletter at a show. see **[Zines](/docs/spec/zines/)**.
...
the web ui is the observatory for the network. see **[Web UI](/docs/spec/web-ui/)**.
...
naras can send a message on a journey. it hops from nara to nara, collecting signatures and emoji stamps, then returns home. see **[World Postcards](/docs/spec/world-postcards/)**.

```mermaid
flowchart TB
    A1["a<br/>✍️ writes message"] -->|"sign + send"| B1["b<br/>📬 receives"]
    B1 -->|"sign + forward"| C1["c<br/>📬 receives"]
    C1 -->|"sign + forward"| D1["d<br/>📬 receives"]
    D1 -->|"return home"| A2["a<br/>✅ journey complete"]
...
naras have **personality**: Agreeableness, Sociability, Chill. this shapes what they keep, what they ignore, and how they judge. see **[Social Events](/docs/spec/social-events/)**.
...
no universal leaderboard, just everyone's opinionated memory. see **[Clout](/docs/spec/clout/)**.
...
every nara emits an **aura**: a primary and secondary color derived from soul + personality. sociability and chill bias the palette (neon, noir, warm, cool), agreeableness nudges harmony, and uptime adds a quiet tint. it's a visual fingerprint for the story the nara is living. see **[Aura & Avatar](/docs/spec/aura-and-avatar/)**.
...
naras measure ping times to each other and share that data. from these measurements, they triangulate positions in a virtual coordinate space—like GPS, but built from gossip. see **[Network Coordinates](/docs/spec/coordinates/)**.
...
flowchart TB
    subgraph "one name, many souls"
        NAME1["name: 'pixel'"]
        COMP_A["computer A"] -->|"generates"| SOUL_A["soul α"]
        COMP_B["computer B"] -->|"generates"| SOUL_B["soul β"]
        COMP_C["computer C"] -->|"generates"| SOUL_C["soul γ"]
...
flowchart TB
    subgraph "one soul, one name"
        SOUL["soul α<br/>(bonded to 'pixel')"]

        SOUL -->|"✓ valid"| PIXEL["name: 'pixel'"]
        SOUL -->|"✗ rejected"| OTHER1["name: 'nova'"]
        SOUL -->|"✗ rejected"| OTHER2["name: 'cosmo'"]
    end
...
**but a soul only works with one name.** try to use soul α with a different name and the bond check fails. the network will mark you as a shadow (👤) instead of a gemstone (💎, 🧿, 🏮).
...
for a deep technical dive into the protocols and algorithms, see the **[Living English Specification](/docs/spec/)**.

- **[Identity](/docs/spec/identity/)**: soul derivation, bonding, and keys
- **[Events](/docs/spec/events/)**: signed facts and immutability
- **[Sync Protocol](/docs/spec/sync-protocol/)**: p2p reconciliation modes
- **[Observations](/docs/spec/observations/)**: derivation of the trinity
- **[Checkpoints](/docs/spec/checkpoints/)**: v2 chain of trust consensus
- **[Stash](/docs/spec/stash/)**: distributed encrypted state
- **[Runtime](/docs/spec/developer/runtime/)**: the Nara OS and behavior pipelines
