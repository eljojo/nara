...
type Attestation struct {
	...
	// About whom
	Subject   types.NaraName `json:"subject"`    // Nara name being attested about
	SubjectID types.NaraID   `json:"subject_id"` // Nara ID (public key hash)

	// What state is being claimed
	Observation NaraObservation `json:"observation"`

	// Who is making the claim
	Attester   types.NaraName `json:"attester"`    // Nara name making the attestation
	AttesterID types.NaraID   `json:"attester_id"` // Nara ID (public key hash)
