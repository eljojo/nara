...
### 2. Encryptor (Self-Encryption)

**Purpose:** Encrypt data that only the owner can decrypt.

**Current Implementation:** `identity_crypto.go:109-174`
```go
...
**Problem:** Currently coupled to identity package. Stash imports it directly.

**Proposed Utility:** Extract and generalize:
```go
// utilities/encryptor.go
package utilities
...
// Create from any 32-byte seed (e.g., derived from Ed25519 private key)
func NewEncryptor(seed []byte) *Encryptor

// Encrypt with random nonce (XChaCha20-Poly1305)
...
// Convenience: seal and return combined nonce+ciphertext
func (e *Encryptor) SealCombined(plaintext []byte) ([]byte, error)

// Convenience: split and open combined blob
func (e *Encryptor) OpenCombined(blob []byte) ([]byte, error)
```

**Current Usages to Replace:**
- `stash_types.go:569-626` - CreateStashPayload, DecryptStashPayload
- `identity_crypto.go:137-174` - EncryptForSelf, DecryptForSelf
...
func (s *StashService) CreatePayload(data *StashData) (*StashPayload, error) {
    serialized, _ := json.Marshal(data)
    compressed, _ := gzip.Compress(serialized)
    nonce, ciphertext, err := s.encryptor.Seal(compressed)
    return &StashPayload{Nonce: nonce, Ciphertext: ciphertext}, err
}
```
